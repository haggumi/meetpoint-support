# 💬 MeetPoint 고객지원 (Support)

안녕하세요!  
**밋포인트(MeetPoint)** 는 서로 다른 출발지에서 **공평한 중간지점을 찾아주는 위치 기반 만남 서비스**입니다.  
친구, 연인, 동료와의 약속 장소를 쉽고 빠르게 정할 수 있도록 도와줍니다.  
더 나은 서비스 제공을 위해 지속적으로 개선하고 있습니다.

---

## 🧭 문의 및 피드백
앱 이용 중 불편사항, 기능 제안, 오류 신고 등은 아래 메일로 보내주세요.  

📩 **dev_haggumi@naver.com**

- 답변 시간: 평일 기준 1~2일 이내  
- 버그 제보 시, 가능한 한 스크린샷 또는 재현 방법을 함께 알려주시면 도움이 됩니다.  

---

## 🛠️ 개발 정보
- **앱 이름:** 밋포인트(MeetPoint)  
- **개발자:** Hayeon Lee  
- **플랫폼:** iOS / Android  
- **카테고리:** 라이프스타일 / 위치기반 서비스  

---

## 🔒 개인정보 처리방침
개인정보 처리방침은 아래 페이지에서 확인하실 수 있습니다.  
👉 [https://haggumi.github.io/meetpoint-support/privacy-policy]
---

© 2025 Hayeon Lee. All rights reserved.
